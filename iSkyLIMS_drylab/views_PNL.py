...
def index(request):
    ...
    if Service.objects.filter(serviceStatus__exact = 'rejected').exists():
        rej_service_objs =  Service.objects.filter(serviceStatus__exact = 'rejected').order_by('serviceOnRejectedDate')
        service_list['rejected'] = []
        for rej_service_obj in rej_service_objs:
            s_info = rej_service_obj.get_service_information()
            s_info.append(rej_service_obj.get_service_requested_user())
            service_list['rejected'].append(s_info)
    ...
    if Service.objects.all().exclude(serviceStatus__exact = 'delivered').exclude(serviceOnApprovedDate = None).exists():
        ...
        for ongoing_services_obj in ongoing_services_objs:
            ...
            service_list['delivered'].append(s_info)

    if Service.objects.filter(serviceStatus__exact = 'approved').exists():
        a_service_objs =  Service.objects.filter(serviceStatus__exact = 'approved').order_by('serviceOnApprovedDate')
        service_list['approved'] = []
        for a_service_obj in a_service_objs:
            s_info = a_service_obj.get_service_information()
            s_info.append(a_service_obj.get_service_requested_user())
            service_list['approved'].append(s_info)

    if Service.objects.filter(serviceStatus__exact = 'queued').exists():
        q_service_objs =  Service.objects.filter(serviceStatus__exact = 'queued').order_by('serviceCreatedOnDate')
        service_list['queued'] = []
        for q_service_obj in q_service_objs:
            s_info = q_service_obj.get_service_information()
            s_info.append(q_service_obj.get_service_requested_user())
            service_list['queued'].append(s_info)

    if Service.objects.filter(serviceStatus__exact = 'in_progress').exists():
        p_service_objs =  Service.objects.filter(serviceStatus__exact = 'in_progress').order_by('serviceCreatedOnDate')
        service_list['in_progress'] = []
        for p_service_obj in p_service_objs:
            s_info = p_service_obj.get_service_information()
            s_info.append(p_service_obj.get_service_requested_user())
            service_list['in_progress'].append(s_info)

    if Service.objects.filter(serviceStatus__exact = 'archived').exists():
        ar_service_objs =  Service.objects.filter(serviceStatus__exact = 'archived').order_by('serviceCreatedOnDate')
        service_list['archived'] = []
        for ar_service_obj in ar_service_objs:
            s_info = ar_service_obj.get_service_information()
            s_info.append(ar_service_obj.get_service_requested_user())
            service_list['archived'].append(s_info)
